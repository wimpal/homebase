...
You are working in **Homebase**, one of three repositories in a household system.
...
- `project-control-heim/status/homebase.md` — what state this repo is in right now
- `project-control-heim/contracts/homebase.tools.yaml` — **the tool surface this repo promises**
...
Homebase is the household operations source of truth for Mimir: inventory, shopping
lists, tasks, recipes (via MCP). The app itself is broader — see the guide below.

- For Mimir-facing work, prefer inventory and shopping list first.
- Keep the MCP layer thin. All logic lives in the domain layer so the web UI and
  MCP drive the same code. Business logic in a tool handler is a bug.
- Tools you are not implementing this session stay `status: planned` in the
  contract. Do not mark something `implemented` because the handler exists.
...
HomeBase helps households track inventory, chores, plants/pets, shopping, calendar events, routines, recipes, budget, deliveries, messaging, and smart-home devices. **This deployment is IKEA / Dirigera** (Trådfri) — see [Smart home (this household)](#smart-home-this-household) below. It is:
...
- **IKEA / Dirigera** — hub on LAN; Zigbee bulbs (plus blinds/outlets/sensors in the home
...
2. **Home Assistant** — optional later for **voice satellites / Wyoming (M6b)** or
...
1. Rewrite `status/homebase.md` to reflect reality **now**. Overwrite, don't append.
...
3. Update the `status:` field of any tool you touched in `contracts/homebase.tools.yaml`.
