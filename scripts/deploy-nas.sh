...
DOCKER_CMD="set -eu && cd '$NAS_PATH' && sudo docker compose up --build -d && sudo docker compose exec -T app npx prisma db push && sudo docker compose logs --tail=30 && sleep 2 && curl -sf http://127.0.0.1:3000/health"
...
if [ "$USE_SCP" -eq 1 ]; then
  ...
  ssh -p "$NAS_SSH_PORT" -t "$REMOTE" "set -eu && mkdir -p '$NAS_PATH' && cd '$NAS_PATH' && tar xzf '$REMOTE_TAR' && rm -f '$REMOTE_TAR' && sudo docker compose up --build -d && sudo docker compose exec -T app npx prisma db push && sudo docker compose logs --tail=30 && sleep 2 && curl -sf http://127.0.0.1:3000/health"
else
