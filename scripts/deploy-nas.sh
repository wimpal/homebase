...
if [ "$USE_SCP" -eq 1 ]; then
  ...
  ssh -p "$NAS_SSH_PORT" "$REMOTE" "set -eu && mkdir -p '$NAS_PATH' && cd '$NAS_PATH' && tar xzf '$REMOTE_TAR' && rm -f '$REMOTE_TAR' && docker compose up --build -d && docker compose exec -T worker npx tsx scripts/migrate-shopping-slots.ts && docker compose exec -T worker npx tsx scripts/migrate-project-work-items.ts && docker compose exec -T worker npx prisma db push --accept-data-loss && docker compose exec -T worker npx tsx scripts/ensure-product-ci-index.ts && docker compose logs --tail=30 && sleep 2 && curl -sf http://127.0.0.1:3000/health"
else
